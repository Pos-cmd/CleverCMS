...
//@ts-expect-error
const rules = computed<Record<keyof LoginParams, App.Global.FormRule[]>>(() => {
...
async function handleSubmit() {
  await validate();
  await authStore.login({email: model.email, password: model.password});
}
...
async function handleAccountLogin(account: Account) {
  await authStore.login({ email: account.userName, password: account.password });
}
</script>
...
<template>
  <NForm ref="formRef" :model="model" :rules="rules" size="large" :show-label="false" @keyup.enter="handleSubmit">
    ...
    <NSpace vertical :size="24">
      ...
      <NButton type="primary" size="large" :loading="authStore.isLoading" round block @click="handleSubmit">
        {{ $t('common.confirm') }}
      </NButton>
