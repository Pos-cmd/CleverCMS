...
import { addColorAlpha, getColorPalette, getPaletteColorByNumber, getRgb } from '@clever/color'
...
export function getNaiveCssVarsRef(keyName: keyof GlobalTheme, propKey: string, darkMode: boolean = false) {
  if (!getCurrentInstance())
    return
  const lightList = getNaiveCssVars(keyName)
  const darkList = getNaiveCssVars(keyName, true)
  const cssVarRef = computed((): string => {
    return darkMode ? darkList[propKey] : lightList[propKey]
  })
